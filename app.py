...
# Optimierte Funktion für gekrümmte Linien (Erdkrümmung/Großkreis)
def create_curved_route(lat1, lon1, lat2, lon2, num_points=50): 
    # Umwandlung in Radiant
    phi1, lam1 = math.radians(lat1), math.radians(lon1)
    phi2, lam2 = math.radians(lat2), math.radians(lon2)
    
    # Winkelabstand (Zentralwinkel) berechnen
    d_phi = phi2 - phi1
    d_lam = lam2 - lam1
    ...
    for i in range(num_points + 1):
        f = i / num_points
        # Zwischenpunkte auf dem Großkreis berechnen (Slerp-Annäherung)
        ...
        phi_f = math.atan2(z, math.sqrt(x**2 + y**2))
        lam_f = math.atan2(y, x)
        
        lats.append(math.degrees(phi_f))
        lons.append(math.degrees(lam_f))
...
with col_comm_text:
    # Berechnung des Multiplikators für Business Flüge
    times_business = 1.0 / COMMERCIAL_FACTOR
    ...
    st.success(f"Vergleich: Mit dem CO₂-Ausstoß eines einzigen Privatflugs könnte Tom Cruise die gleiche Strecke **{int(times_business)} Mal** mit einem Business-Flug zurücklegen.")
